...
python -m price_monitor.main analyze     # → reports/*.{md,json,xlsx}
...
| Analysis | `analysis/` | `TrendAnalyzer` (moving average, all-time low), `ValueRanker` (price/spec/trend composite score 0-100, **grouped by spec**: memory_type → form_factor → capacity → speed → CL tier), `ReportGenerator` (JSON/Markdown/Excel single-sheet with spec column/Feishu card). |
...
MVP 全链路已于 2026-04-27 验证通过。248 tests passing.

- **急需积累价格趋势**：每天手动 `bash scripts/daily_crawl.sh` 或后台 `python -m price_monitor.main scheduler`。≥3 个历史点后趋势分析才能判断涨跌。
- **数据质量改进**：CPU 型号黑名单（防 9800X3D 误识别为频率）、中文"笔记本"关键词识别 SO-DIMM、CL 覆盖率仅 ~40%。
